...
Array.prototype.even = function(){
    return this.filterNonInts().filter(num => num % 2 === 0);
  }
  ...
  Array.prototype.under = function(x){
    return this.filterNonInts().filter(num => num < x);
  }
  ...
  Array.prototype.inRange = function(min,max){
    return this.filterNonInts().filter(num => num >= min && num <= max);
  }
  
  Array.prototype.filterNonInts = function() {
    return this.filter(val => Number(val) === val && Math.floor(val) === val);
  }
