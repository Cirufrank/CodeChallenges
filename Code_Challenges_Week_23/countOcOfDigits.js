...
function List(){
    this.countSpecDigits=function(integersList, digitsList){
      integersList = integersList.map(int => (int < 0) ? Math.abs(int) : int);
      const stringIntListNums = integersList.join("");
      const digAndOccArray = [];
      digitsList.forEach(digit => {
        const digRegex = new RegExp(String(digit), "g");
        const occurencesOfDigit = (stringIntListNums.match(digRegex)) ? stringIntListNums.match(digRegex).length : 0;
        digAndOccArray.push([digit, occurencesOfDigit]);
      });
      return digAndOccArray;
    }
