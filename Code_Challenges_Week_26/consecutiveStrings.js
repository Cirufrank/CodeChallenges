...
function longestConsec(strarr, k) {
    ...
    if (k === 1) return strarr.slice().sort((a, b) => b.length - a.length)[0];
    let pointer = 0;
    let largestConcatString = strarr.slice(pointer, pointer + k).join("");
    pointer += 1;
    while (pointer + k <= strarr.length) {
      const currentConcatString = strarr.slice(pointer, pointer + k).join("");
      if (largestConcatString.length < currentConcatString.length) largestConcatString = currentConcatString;
      pointer += 1;
    }
  return largestConcatString;
}
