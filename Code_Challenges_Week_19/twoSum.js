...
function twoSum(numbers, target) {
    const mapOfNumsAndIndxs = new Map();
    numbers.forEach((num, index) => {
      mapOfNumsAndIndxs.set(num, index);
    });
    
    for (let index = 0; index < numbers.length; index += 1) {
      let currentNum = numbers[index];
      let numberNeeded = target - currentNum;
      if (mapOfNumsAndIndxs.has(numberNeeded) && index !== mapOfNumsAndIndxs.get(numberNeeded)) return [index, mapOfNumsAndIndxs.get(numberNeeded)];
    }
  }
